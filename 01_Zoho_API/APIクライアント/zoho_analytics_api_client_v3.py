...
# -*- coding: utf-8 -*-
"""
Zoho Analytics API v2 クライアント（修正版）
VERSANTコーチングレポート実行用
"""
...
class ZohoAnalyticsAPIv3:
    def __init__(self, access_token=None, workspace_id=None, org_id=None):
        ...
        self.org_id = org_id or os.getenv('ZOHO_ANALYTICS_ORG_ID', '775100530')  # 正しい組織ID
    ...
    def execute_query_get(self, query, output_format='json'):
        """
        SQLクエリをGETリクエストで実行（短いクエリ用）
    ...
    def get_workspaces(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                shared_workspaces = data.get('data', {}).get('sharedWorkspaces', [])
                print(f"✅ ワークスペース情報取得完了: 所有{len(workspaces)}件, 共有{len(shared_workspaces)}件")
                return workspaces + shared_workspaces
            else:
...
def main():
    ...
    メイン実行関数
    """
    print("=== VERSANTコーチングレポート実行（修正版）===")
    
    # APIクライアントの初期化
    try:
        client = ZohoAnalyticsAPIv3()
    ...
    if workspaces:
        ...
        for ws in workspaces:
            print(f"   - {ws['workspaceName']} (ID: {ws['workspaceId']}, Org: {ws['orgId']})")
    ...
    for sql_file in sql_files:
        ...
        # クエリの長さに応じてGETまたはPOSTを使用
        if len(query) < 2000:
            results = client.execute_query_get(query, output_format='json')
        else:
