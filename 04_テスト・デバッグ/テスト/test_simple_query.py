...
def test_simple_query(workspace_id, org_id, access_token, sql_file_path):
    """
    シンプルなクエリをテスト
    """
    headers = {
    ...
    print(f"🔄 シンプルクエリ実行中: {sql_file_path}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'data' in data and isinstance(data['data'], list):
                ...
                output_file = f"versant_simple_{timestamp}.json"
...
def main():
    ...
    # シンプルなクエリをテスト
    result = test_simple_query(workspace_id, org_id, access_token, "versant_coaching_report_simple.sql")
