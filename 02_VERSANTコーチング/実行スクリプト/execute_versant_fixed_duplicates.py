#!/usr/bin/env python3
"""
VERSANT Coaching Report Fixed Duplicates Execution Script
Executes the VERSANT coaching report with duplicate counting prevention
"""
...
def execute_versant_fixed_duplicates():
    """Execute the VERSANT coaching report with duplicate counting prevention"""
    ...
    # Read the fixed SQL query
    sql_file_path = "versant_coaching_report_fixed_duplicates.sql"
    
    try:
        with open(sql_file_path, 'r', encoding='utf-8') as f:
            sql_query = f.read()
        
        print(f"📋 Executing VERSANT coaching report with duplicate prevention...")
        ...
        print(f"🔧 Fixed: Prevents duplicate counting from multiple arrangement records")
        print()
        ...
        if result:
            ...
            output_filename = f"versant_fixed_duplicates_result_{timestamp}.json"
            
            # Save results to JSON file
            with open(output_filename, 'w', encoding='utf-8') as f:
                json.dump(result, f, ensure_ascii=False, indent=2)
            
            print(f"✅ VERSANT coaching report with duplicate prevention executed successfully!")
            ...
            if result.get('data'):
                ...
                for record in result['data']:
                    ...
                    products = record.get('手配商品名', 'Unknown')
                ...
                for record in result['data']:
                    if record.get('メールアドレス') == target_email:
                        ...
                        product_names = record.get('手配商品名', 'Unknown')
                ...
                # Verify Answer table counts are not inflated
                print(f"\n🔍 Answer table count verification:")
                print(f"  This version uses subquery to prevent duplicate counting")
                print(f"  Each student's Answer records are counted only once")
                print(f"  Multiple arrangement records won't inflate the counts")
...
if __name__ == "__main__":
    execute_versant_fixed_duplicates() 