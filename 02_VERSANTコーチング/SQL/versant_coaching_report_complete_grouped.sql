-- VERSANTコーチング 完全版レポート（受講生グループ化）
-- 1人に複数レコードが出る問題を解決するため受講生でグループ化
...
SELECT
    ...
    c."メール" as "メールアドレス",
    acc."取引先名" as "会社名",
    GROUP_CONCAT(DISTINCT p."商品名" SEPARATOR ', ') as "商品名",
    MIN(ar."学習開始日") as "学習開始日",
    
    -- 直近3週間の日別VERSANTコーチング回答数（日付表示）
...
LEFT JOIN "取引先" acc ON c."取引先ID" = acc."Id"
LEFT JOIN "商品マスタ" p ON ar."商品ID" = p."Id"
